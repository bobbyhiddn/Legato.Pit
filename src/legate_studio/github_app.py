...
def _get_app_config() -> dict:
    ...
    if _app_config is None:
        ...
        if not all([app_id, client_id, client_secret]):
            raise ValueError(
                "GitHub App not configured. Set GITHUB_APP_ID, GITHUB_APP_CLIENT_ID, and GITHUB_APP_CLIENT_SECRET"
            )
        ...
        if private_key_path and Path(private_key_path).exists():
            ...
            raise ValueError(
                "GitHub App private key not found. Set GITHUB_APP_PRIVATE_KEY_PATH or GITHUB_APP_PRIVATE_KEY"
            )
...
def get_installation_for_user(github_login: str) -> dict | None:
...
_token_manager: InstallationTokenManager | None = None
