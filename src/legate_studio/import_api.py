...
def upload_zip():
    ...
    if len(zip_content) > MAX_ZIP_SIZE:
        return jsonify({"error": f"File too large. Maximum size is {MAX_ZIP_SIZE // (1024 * 1024)}MB."}), 400
    ...
    try:
        ...
        if len(job.files) == 0:
            return jsonify({"error": "No markdown files found in ZIP"}), 400

        logger.info(
            f"User {user_id} uploaded ZIP with {len(job.files)} files, job {job.job_id}, category_mode={category_mode}"
        )
...
def list_jobs():
    ...
    jobs = list_user_jobs(user_id)
    return jsonify({"jobs": [job_to_dict(j, include_files=False) for j in jobs]})
...
def update_file_classification(job_id: str, file_idx: int):
    ...
    return jsonify({"success": True, "file": file_to_dict(file)})
...
def confirm_import(job_id: str):
    ...
    if job.status != "classified":
        return jsonify({"error": f"Job must be classified before confirming. Current status: {job.status}"}), 400
...
def preview_job(job_id: str):
    ...
    job = get_job(job_id, user_id)
    if not job:
        return render_template("error.html", title="Not Found", message="Import job not found"), 404
